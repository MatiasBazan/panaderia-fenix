...
type Props = {
    ...
    /**
     * Monto de la seña, en pesos. Sólo lo pasan las pantallas a las que el
     * controller se lo manda (cotización y gracias): no viaja en las props
     * compartidas porque ahí no van importes. 0 o ausente deja el texto sin
     * número, que es lo correcto donde todavía no hay pedido concreto.
     */
    senaMonto?: number;
    className?: string;
};
                    ...
                    <li className="flex items-start gap-3 text-sm leading-relaxed text-texto-medio">
                        ...
                        <span>
                            <span className="font-medium text-texto">
                                {senaMonto > 0
                                    ? `Se confirma con una seña de $${senaMonto.toLocaleString('es-AR')}.`
                                    : 'Se confirma con una seña.'}
                            </span>{' '}
                            {senaMonto > 0
                                ? 'Te pasamos cómo abonarla cuando confirmemos los precios.'
                                : 'Te pasamos el monto junto con los precios, y ahí coordinamos cómo abonarla.'}
                        </span>
                    </li>
                )}
            </ul>
